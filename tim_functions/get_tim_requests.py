...
def download_products_file(token):
    products_file = Path(__file__).parent.parent.joinpath(
        "databases/tim/new_products.csv"
    )
    schema = """
        query{
    ...
    while status != 200:
        responce = requests.post(
            url="https://www.tim.pl/graphql", json={"query": schema}, headers=header
        )
...
def download_invertory_file(token):
    inventory_file = Path(__file__).parent.parent.joinpath(
        "databases/tim/inventory.csv"
    )
    schema = """
            query{
...
def download_prices_file(token):
    prices_file = Path(__file__).parent.parent.joinpath("databases/tim/new_prices.csv")
    schema = """
        query{
        priceList{link}
        }
