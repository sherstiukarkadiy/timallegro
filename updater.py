...
import json
import time
from tim_functions.get_tim_requests import (
    download_invertory_file,
    download_prices_file,
...
CATEGORIES = Path(__file__).parent.joinpath("categories.json")
...
def update_database():
    ...
    with open(
        download_prices_file(tim_token),
        "r",
    ) as file:
        reader = csv.reader(file)
        price_dict = {
            row[1]: (float(row[-1].replace(",", ".")) if row[-1] else 0)
            for row in reader
        }
